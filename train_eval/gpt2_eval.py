...
from libs import loggers, get_between
...
def evaluate(model, dataset, batch_size, epochs, data_func=lambda x: x):
    ...
    return perplexity, np.array(perplexities), np.array(losses)
...
def classifier_eval_one_epoch(
        ...
        num_samples: int,
        data_process_func,
        tokenizer: tfm.PreTrainedTokenizer):
    device = next(model.parameters()).device
    result = pd.DataFrame(columns=['id', 'e1', 'e2', 'full_sent',
                                   'text_bt', 'pred_label', 'pred_logit', 'gt_label',
                                   'prob', 'e1i', 'e2i'])
    sample_logger = loggers.sample_logger
    res_data = {k: [] for k in result.columns}
    # print('d', dataloader.batch_size)
    for step, raw in enumerate(dataloader):
        # print(step, raw)
        data = data_process_func(raw)
        if data is None:
        ...
        for i in range(len(data['input_ids'])):
            ...
            if sent.shape[-1] == 0:
                continue
            input_data = {'input_ids': sent[None, :], 'attention_mask': attn_mask[None, :],
                          'position_ids': pos_ids[None, :]}
            e1l, e2l = tokenizer.decode(e1.tolist()), tokenizer.decode(e2.tolist())
            # print(e1l, e2l, sentl)
            print('sampling {}, {}'.format(e1l, e2l))

            label, logit = sample_classifier_sequence(model, input_data)
            print('gen_time: {}'.format(time.time() - gen_time))
            # print(sent)

            sent = sent[len(e1) + len(e2):]
            sentl = tokenizer.decode(sent.tolist())
            eval_time = time.time()
            # a, b = get_between(e1l, e2l, sentl, inclusive=False)
            idx = raw[i][-1]

            # print(e1l, e2l)
            # print(tokenizer.decode(raw[i][2]))
            # print(sentl)
            res_data['id'].append(raw[i][-1][0])
            res_data['e1'].append(e1l)
            res_data['e2'].append(e2l)
            res_data['full_sent'].append(tokenizer.decode(raw[i][2]))
            res_data['text_bt'].append(sentl)
            res_data['pred_label'].append(label[0][1])
            res_data['pred_logit'].append(logit[0])
            res_data['gt_label'].append(raw[i][-1][-1])
            res_data['prob'].append(label[0][0])
